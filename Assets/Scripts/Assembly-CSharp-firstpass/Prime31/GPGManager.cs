...
namespace Prime31
{
	public class GPGManager : AbstractManager
	{
		...
		{
			if (GPGManager.loadPlayerStatsSucceededEvent != null)
			{
				GPGManager.loadPlayerStatsSucceededEvent(Json.decode<Dictionary<string, object>>(json));
			}
		}

		private void loadPlayerStatsFailed(string error)
		{
			GPGManager.loadPlayerStatsFailedEvent.fire(error);
		}

		private void authTokenFetchResult(string token)
		{
			GPGManager.authTokenFetchResultEvent.fire(token);
		}
		...
		private void videoCaptureNotSupported(string error)
		{
			if (GPGManager.videoCaptureNotSupportedEvent != null)
			{
				GPGManager.videoCaptureNotSupportedEvent(error);
			}
		}
