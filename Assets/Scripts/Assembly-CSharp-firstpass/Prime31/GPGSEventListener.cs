...
namespace Prime31
{
	public class GPGSEventListener : MonoBehaviour
	{
		...
		private void loadPlayerStatsSucceededEvent(Dictionary<string, object> stats)
		{
			Debug.Log("loadPlayerStatsSucceededEvent");
			Utils.logObject(stats);
		}

		private void loadPlayerStatsFailedEvent(string error)
		{
		...
		private void snapshotListUserSelectedSnapshotEvent(GPGSnapshotMetadata metadata)
		{
			...
			SendOb.SendMessage("toload", SendMessageOptions.DontRequireReceiver);
		}
		...
		private void loadSnapshotSucceededEvent(GPGSnapshot snapshot)
		{
			...
			string @string = Encoding.UTF8.GetString(snapshot.snapshotData);
			if (snapshot.hasDataAvailable)
			{
				SendOb.SendMessage("loadend", @string, SendMessageOptions.DontRequireReceiver);
			}
			else
			{
				SendOb.SendMessage("loadend_error", SendMessageOptions.DontRequireReceiver);
			}
		...
		{
			SendOb.SendMessage("saveend", SendMessageOptions.DontRequireReceiver);
