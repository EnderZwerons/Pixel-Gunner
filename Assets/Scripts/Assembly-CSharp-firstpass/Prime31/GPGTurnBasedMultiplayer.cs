...
namespace Prime31
{
	public class GPGTurnBasedMultiplayer
	{
		...
		public static void showPlayerSelector(int minPlayersToPick, int maxPlayersToPick, int variant = 1)
		{
			if (Application.platform == RuntimePlatform.Android)
			{
				_plugin.Call("showPlayerSelector", minPlayersToPick, maxPlayersToPick, variant);
			}
		}
