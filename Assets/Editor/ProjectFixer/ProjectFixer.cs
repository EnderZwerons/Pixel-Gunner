...
public class ProjectFixer : Editor
{
    ...
    public static void FixPremultiplied()
    {
        foreach (Material mat in (from guid in AssetDatabase.FindAssets("t:Material") where AssetDatabase.LoadAssetAtPath<Material>(AssetDatabase.GUIDToAssetPath(guid)).shader.name == "Unlit/Premultiplied Colored" select AssetDatabase.LoadAssetAtPath<Material>(AssetDatabase.GUIDToAssetPath(guid))).ToList())
        {
